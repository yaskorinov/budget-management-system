...
* **Ролики после операций**: `gifs/topup.mp4` и `gifs/buy.mp4` уходят перед
  карточкой (`sendAnimation`, mp4 Telegram показывает как гифку). Первый раз
  файл загружается, дальше отправляется по `file_id`. Ролик — украшение:
  любая ошибка с ним не мешает записать операцию и показать карточку.
