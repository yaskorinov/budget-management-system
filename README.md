...
хотя с хоста `curl -x` работает. Лечится оверлеем, который переводит бот на
стандартный мост:
...
и адресом `PROXY_URL=socks5://172.17.0.1:1080` в `.env`. Публикация порта
не меняется, наружу по-прежнему смотрит только `127.0.0.1:8080`, но собственной
сети у контейнера не остаётся — оверлей с Caddy к нему не подойдёт, тот ходит
к боту по имени. С nginx на хосте всё в порядке.
