...
или «внёс пять тысяч». Расшифровку бот показывает перед записью, чтобы ослышку
было видно. В группе реагирует только на голосовые в ответ на своё сообщение.
...
**Про LLM.** По умолчанию — OpenRouter и `nvidia/nemotron-3.5-lightning:free`.
Провайдер `openai_compat` подходит любому OpenAI-совместимому эндпоинту
`/v1/chat/completions`: OpenRouter, OpenAI, свой прокси, локальные vLLM/Ollama;
`anthropic` ходит через официальный SDK. При любой ошибке — тихий фолбэк на
словарь ключевых слов, бот не встаёт.

Для `/voice` нужна вторая модель: основная принимает только текст. По умолчанию
`nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free` — она тоже бесплатная и
принимает аудио.

**Про бесплатные модели OpenRouter.** Они бесплатны по токенам, но не в вакууме:

* при **отрицательном балансе** аккаунта приходит `402 Payment Required` — даже
  на моделях с суффиксом `:free`;
* лимит — **50 запросов в сутки**, если куплено меньше 10 кредитов, и **1000**,
  если 10 и больше; плюс 20 запросов в минуту. Превышение отдаёт `429`.

Категоризация при любой ошибке молча уходит в словарь, поэтому сбой заметен не
сразу — проверяйте `python scripts/check_llm.py`, он делает пробный запрос и
печатает ответ сервера целиком.
