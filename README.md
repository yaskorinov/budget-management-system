...
* **Разметка сообщений** — MarkdownV2. Собирается функциями из `app/bot/markup.py`,
  а не руками в f-строках: всё, что приходит извне (имена, названия бюджетов,
  описания покупок), экранируется по правилам Telegram. Иначе участник с `(` или
  `.` в имени ломает сообщение целиком — Telegram отвергает его с ошибкой разбора
  сущностей. `tests/md_check.py` разбирает разметку так же, как Telegram, и через
  него проходит каждое сообщение в тестах.
